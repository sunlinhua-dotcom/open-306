...
class Note:
    ...
    # 搜索笔记
    async def search_notes(self,keyword: str, page: int = 1, page_size: int = 40, sort: str = "popularity_descending", note_type: int = 0) -> aiohttp.ClientResponse:
        """搜索笔记

        Args:
            keyword: 搜索关键词
            page: 页码  默认 1 (请从1开始)
            page_size: 每页大小 默认 40
            sort: 排序方式 "general"综合 / "popularity_descending"最热(默认) / "time_descending"最新
            note_type: 笔记类型 默认 0
        """
        ...
        return await self.session.request("post", url=uri, data=data)
    ...
    async def search_all_notes(self, keyword: str, max_pages: int = 5, page_size: int = 40,
                                sort: str = "popularity_descending", note_type: int = 0,
                                crawl_interval: float = 2.0, with_detail: bool = False) -> list:
        """搜索笔记 — 自动翻页，获取多页结果

        Args:
            keyword: 搜索关键词
            max_pages: 最大翻页数（默认5页，每页40条 = 最多200条）
            page_size: 每页大小，默认40
            sort: 排序方式 "general"综合 / "popularity_descending"最热 / "time_descending"最新
            note_type: 笔记类型，默认0（全部）
            crawl_interval: 翻页间隔秒数（防风控，默认2秒）
            with_detail: 是否同时获取笔记详情（更慢但数据更完整）

        Returns:
            list: 所有笔记的列表，每条包含 note_id, xsec_token, display_title 等字段
        """
        all_items = []

        for page in range(1, max_pages + 1):
            try:
                res = await self.search_notes(keyword, page=page, page_size=page_size,
                ...
                if not items:
                    print(f"[搜索] 第{page}页无结果，停止翻页")
                    break

                all_items.extend(items)
                print(f"[搜索] 第{page}/{max_pages}页: 获取 {len(items)} 条，累计 {len(all_items)} 条")

                if not has_more:
                    print(f"[搜索] 没有更多结果，共 {len(all_items)} 条")
                    break

                if page < max_pages:
                    # 随机延迟防风控
                    delay = crawl_interval + random.uniform(0.5, 1.5)
                    await asyncio.sleep(delay)

            except Exception as e:
                print(f"[搜索] 第{page}页失败: {e}")
                break

        # 可选：批量获取详情
        if with_detail and all_items:
            print(f"[搜索] 开始获取 {len(all_items)} 条笔记详情...")
            detailed_items = []
            for i, item in enumerate(all_items):
                ...
                if note_id and xsec_token:
                    try:
                        detail_res = await self.note_detail(note_id, xsec_token)
                        detail_data = await detail_res.json()
                        item['_detail'] = detail_data.get('data', {})
                        print(f"[详情] {i+1}/{len(all_items)}: {note_id}")
                    except Exception as e:
                        print(f"[详情] {i+1}/{len(all_items)}: {note_id} 失败 - {e}")
                        item['_detail'] = None

                    await asyncio.sleep(crawl_interval + random.uniform(0.3, 1.0))
    ...
    async def search_all_user_notes(self, user_id: str, max_pages: int = 10, num: int = 30,
                                     crawl_interval: float = 2.0) -> list:
        """获取用户所有笔记 — 自动翻页

        Args:
            user_id: 用户ID
            max_pages: 最大翻页数（默认10页，每页30条 = 最多300条）
            num: 每页数量，默认30
            crawl_interval: 翻页间隔秒数
        ...
        for page in range(1, max_pages + 1):
            try:
                res = await self.search_user_notes(user_id, num=num, cursor=cursor)
                data = await res.json()
                ...
                if not notes:
                    print(f"[用户笔记] 第{page}页无结果，停止翻页")
                    break

                all_notes.extend(notes)
                print(f"[用户笔记] 第{page}/{max_pages}页: 获取 {len(notes)} 条，累计 {len(all_notes)} 条")

                if not has_more:
                    print(f"[用户笔记] 没有更多，共 {len(all_notes)} 条")
                    break
